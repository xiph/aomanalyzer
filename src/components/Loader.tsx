...
> {
  ...
  load(decoderPaths: string[], decoderNames: string[], videoPaths: string[]) {
    ...
    Promise.all(decoderPaths.map((path) => Decoder.loadDecoder(path)))
      .then((decoders) => {
        this.decoders = decoders;
        this.setState({ status: 'Downloading Files' } as any);
        Promise.all(videoPaths.map((path) => downloadFile(path)))
          .then((bytes) => {
            const decodedFrames = [];
  ...
  decodeFrames(decoder: Decoder, count: number): Promise<AnalyzerFrame[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      framePromises = framePromises.map((p) =>
        p
          .then((x) => {
            if (x) {
    ...
    if (this.props.split) {
      return (
        <div className="maxWidthAndHeight">
          <SplitView
            mode={this.props.split - 1}
            onDecodeAdditionalFrames={this.decodeAdditionalFrames.bind(this)}
            groups={this.state.frames}
      ...
      return (
        <div className="maxWidthAndHeight">
          <AnalyzerView
            ...
            groups={this.state.frames}
            groupNames={this.state.groupNames}
            playbackFrameRate={this.state.playbackFrameRate}
            blind={this.props.blind}
            decoderVideoUrlPairs={this.props.decoderVideoUrlPairs}
          ></AnalyzerView>
        </div>
      );
    }
